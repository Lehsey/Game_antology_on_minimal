...
class Nim3(QWidget):
                    ...
                    if self.man_t == 0:
                        ...
                        if self.whichHeap > len(self.heap) or self.whichHeap <= 0:
                            self.Cmd.appendPlainText(
                                "Ты ошибся дружок-пирожок, попробуй снова \n")
                            self.Inp.clear()
                        else:
                            self.Cmd.appendPlainText(
                                "Введите колчество камней, которые возьмёте:")
                            self.Inp.clear()
                            self.man_t = 1
                    else:
    ...
    def AI(self):
        ...
        if max(self.heap) ^ min(self.heap) != 0:
            ...
            self.heap.insert(self.heap.index(
                self.choice(self.heap)), max(self.heap) - 1)
            self.heap.remove(max(self.heap))
        ...
        if self.Win:
            ...
            self.Cmd.appendPlainText(
                "Введите кучу, в которой хотите играть цифрой, например: 1")
    ...
    def peat_cond(self):
        ...
        if self.heap:
            ...
            self.Win = True
            self.over = GameOver(self.game, self.who)
            self.over.show()
